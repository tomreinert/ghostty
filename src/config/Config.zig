...
test "sidebar options: defaults and parsing" {
    const testing = std.testing;
    const alloc = testing.allocator;

    {
        var cfg = try Config.default(alloc);
        defer cfg.deinit();
        try testing.expect(cfg.@"sidebar-git");
        try testing.expect(cfg.@"sidebar-fields" == null);
    }

    {
        ...
        try testing.expect(!cfg.@"sidebar-git");
        try testing.expectEqualStrings("title,status", cfg.@"sidebar-fields".?);
    }
}
